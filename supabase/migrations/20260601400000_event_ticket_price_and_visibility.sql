-- Per-event ticket price + "show remaining seats" toggle.
-- Paid events (ticket_price_cents > 0) block table bookings during their dates.
-- Free events (NULL or 0) do not block tables.
-- show_remaining_seats lets each event hide capacity for marketing reasons (e.g., new event launch).

alter table public.business_events
  add column if not exists ticket_price_cents integer,
  add column if not exists show_remaining_seats boolean not null default true;

comment on column public.business_events.ticket_price_cents is
  'Per-ticket price in cents. NULL or 0 = free / RSVP-only. >0 = paid event — blocks free table bookings on same date.';
comment on column public.business_events.show_remaining_seats is
  'When true, the public booking form shows "X seats left" badges. Turn off for events where you do not want guests to see capacity.';

-- Refresh public RPC to expose new fields.
