-- Show schedule: one-off extra nights and dark (no-show) nights per show, on top of
-- the weekly run_weekdays. Also formalises run_weekdays/show_time, which only ever
-- existed in production. Partner-link RPC re-created to honour the new columns.
begin;

alter table public.show_products
  add column if not exists run_weekdays integer[],
  add column if not exists show_time time,
  add column if not exists run_dates date[] not null default '{}',
  add column if not exists dark_dates date[] not null default '{}';

comment on column public.show_products.run_dates is 'Extra one-off nights this show runs (in addition to run_weekdays).';
comment on column public.show_products.dark_dates is 'Dates this show does NOT run even though run_weekdays says it would.';
...
begin
  ...
  -- Also honours one-off extra nights (run_dates) and dark nights (dark_dates).
  if v_booking.show_date < v_today or v_booking.show_date > (v_today + interval '12 months')::date
    or coalesce(v_booking.show_date = any(v_product.dark_dates), false)
    or (not coalesce(v_booking.show_date = any(v_product.run_dates), false)
      and not coalesce(v_product.run_weekdays @> array[extract(dow from v_booking.show_date)::integer], false)
