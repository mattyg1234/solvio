-- Custom intake questions per hosted event (e.g. "How many children?", "Dietary needs?").
-- Stored as JSONB so the question set lives with the event and travels with recurring occurrences.
-- Also surfaces venue-wide appointment_questions stored in businesses.booking_flow_details.
...
-- Refresh public RPC to expose custom_questions on each event.
