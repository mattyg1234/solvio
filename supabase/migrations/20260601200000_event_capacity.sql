-- Event capacity: stop overbooking hosted events by tracking max attendees per event.

alter table public.business_events
  add column if not exists capacity integer;

comment on column public.business_events.capacity is
  'Max attendees for this hosted event. NULL = unlimited (default for legacy rows).';

-- Refresh the public RPC to include capacity + current booked count per event.
...
begin
  ...
  -- Events now include `capacity` + `booked_count` so the public form can show "X seats left".
  select coalesce(jsonb_agg(jsonb_build_object(
