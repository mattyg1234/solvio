...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: primaryBiz } = await supabase
    .from("businesses")
    .select("platform_capabilities,onboarding_completed_at,booking_flow_completed_at")
    .eq("owner_id", user.id)
  ...
  const bookingSetupComplete = Boolean(primaryBiz?.booking_flow_completed_at);
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <div className="flex min-h-screen">
        <aside className="sticky top-0 hidden h-screen w-[17rem] shrink-0 overflow-hidden border-r border-[#ebe7f7]/90 md:block">
          <DashboardSidebar capabilities={capabilities} bookingSetupComplete={bookingSetupComplete} />
        </aside>
        ...
        <div className="flex min-h-screen flex-1 flex-col pb-[5.75rem] md:pb-0">
          ...
          <DashboardMobileNav capabilities={capabilities} bookingSetupComplete={bookingSetupComplete} />
        </div>
      </div>
    </div>
  );
