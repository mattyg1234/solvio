...
export async function getBusNightOrderAction(
  ...
  const [{ data: orders, error }, { data: stops }, { data: busOrders }] = await Promise.all([
    ctx.supabase
...
export async function saveBusNightOrderAction(
  ...
  const ids = [...new Set((stopIds ?? []).map((s) => String(s ?? "").trim()).filter((s) => UUID_RE.test(s)))];
  if (!ids.length) return { ok: false, message: "Nothing to save — no stops in this order." };
